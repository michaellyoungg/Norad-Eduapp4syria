...
//	bool impatient = false;
	...
	{
		if (_levels [levelId] == null) {
			_levels [levelId] = XMLTool.LoadLevelXML (levelId);
		}
	...
	{
		get {
			return getLevel(mGameplayPosition.LevelIndex);
	...
	{
		get {
			return getLevel(mGameplayPosition.LevelIndex).Segments [mGameplayPosition.SegmentIndex];
		}
	}
...
//			AudioController.Instance.PlaySound (SoundTimeup, .5f);
			...
			if(shildBoosterDuration > 0) {
				shildBoosterDuration -= Time.deltaTime;
			}
...
//		if (addTreasureChest ()) {
	...
	public void LoadSegment ()
	{
		...
		Timer.Instance.Create (new Timer.TimerCommand (Time.time, 0.01f, Countdown, GameState.SegmentIdle));
...
*/
	...
	bool HasNextSegment()
	{
//		return mGameplayPosition.SegmentIndex + 1 <  Levels [mGameplayPosition.LevelIndex].Segments.Length;
		return mGameplayPosition.SegmentIndex + 1 <  getLevel(mGameplayPosition.LevelIndex).Segments.Length;
	}
...
//		UserInfo.Instance.SetLevelScoreIfNewRecord(CurrentLevelIndex, CurrentLevelScore);
	...
	public int CurrentLevelIndex
	{
		...
		set {
			...
			UserInfo.Instance.SetLastPlayingLevel (GameplayController.Instance.CurrentLevelIndex);
		}
	}
				...
				)
				{
					...
					if (letter.stone.value == BoosterController.letterName) {
						...
						if (CurrentSegment.MonsterRequiredLetters.Contains (dragedLetter.stone.value) && CurrentSegment.MonsterRequiredLetters.Contains (letter.stone.value)) {
							dragedLetter.addCollectLetter (letter.stone);
...
//							letter.SetVisible (false);
	...
	public void onStatusCheatClick()
	{
		if (CurrentMonster != null) {
			...
			StatusController.init (CurrentMonster);
		}
	}
