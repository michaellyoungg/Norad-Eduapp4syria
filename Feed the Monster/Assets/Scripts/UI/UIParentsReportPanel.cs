...
public class UIParentsReportPanel : MonoBehaviour {
	...
	public string background = "BG_Splash";
	...
	void OnEnable() {
		...
		Analitics.Instance.treckScreen ("Parents Report");
	...
	void recivedScores()
	{
		...
		foreach(Level lvl in GameplayController.Instance.getLevelList()) {

			if (lvl != null) {
				int stars = UserInfo.Instance.GetLevelStars(lvl.levelId - 1);
	...
	void calculateScores()
	{
		...
		int highestLevelOpenIndex = UserInfo.Instance.GetHighestOpenLevel ();
