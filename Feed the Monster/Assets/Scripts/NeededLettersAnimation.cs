...
//			letter = RTL.Fix(letter);
		...
		case MonsterInputType.LetterInWord:
			for (int i=0; i< GameplayController.Instance.CurrentSegment.MonsterAllLetters.Length; i++) {
...
//				letter = RTL.Fix(letter);
		...
		case MonsterInputType.Word:
			...
			for (int i = 0; i < GameplayController.Instance.CurrentSegment.MonsterRequiredLetters.Length; i++) {
